...
export default function Navbar({ onNav, route }) {
  ...
  // Dynamic background animation like your original
    ...
    >
      ...
      <div className="relative z-[1000]">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 py-3 md:py-4">
          ...
          {/* DESKTOP MENU — ORIGINAL ANIMATION RESTORED */}
          <nav className="hidden md:flex items-center gap-1">
            {navItems.map((item, i) => (
              <motion.button
                ...
                onClick={() => onNav(item)}
                initial={{ opacity: 0, y: -20 }}         // original animation
          ...
          {/* MOBILE HAMBURGER — FIXED CLOSE VISIBILITY */}
          <motion.button
        ...
        {open && (
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 w-[280px] sm:w-[320px] z-[1500] bg-black/50 backdrop-blur-2xl border-l border-white/20 p-8 pt-24"
          >
            {navItems.map((item, i) => (
              <motion.button
                key={item}
                ...
                initial={{ opacity: 0, y: 40 }}            // slide up
                animate={{ opacity: 1, y: 0 }}
